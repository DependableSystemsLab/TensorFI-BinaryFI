...
for i in index:
    ...
    # Note: This is very time-consuming, so you might want to go the TensorFI/faultTypes.py 
    # to configure the number of data items to be injected. 
    # e.g., change the len(val) in "if(indexOfInjectedData < len(val)):" into values you want to inject (e.g., 100) 
    # in our sc'19 paper, we choose 100 data items per operator.
    while(ti.faultTypes.isKeepDoingFI):
        ...
        print(i, totalFI, ti.faultTypes.indexOfInjectedData, ti.faultTypes.indexOfInjectedBit)
